...
def sample_grumbel(placeholder, eps=1e-10):
    """Samples Grumbel(0, 1) values into the placeholder"""
...
def sample_grumbel_softmax(placeholder, logits, tau, **kwargs):
    """Samples values from a grumbel softmax
    ...
    """
    g = sample_grumbel(placeholder, **kwargs)
    return torch.nn.functional.softmax((logits + g) / tau, dim=-1)

def sample_grumbel_max(placeholder, logits, **kwargs):
    """Samples values from a grumbel max
