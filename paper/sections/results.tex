...
\end{tabular}
\end{table}
\todo{should extrapolation be $U[2,6]$?}

The training set is sampled from the interpolation range throughout training. The extrapolation range is defined as 2048 samples picked for each seed at the start of training.\todo{should we define valid and test sets? maybe we should make one BIG test set and have all models run on that}
...
Because the interpolation and extrapolation errors are quite noisy, even for a near perfect solution. The median over the last 100 measurements is reported\todo{use the 2048 as a validation set and just pick the best instead. Then report test results for those}.

A model is considered a success if the extrapolation median is less than $\epsilon = 0.2$. This value was acquired by inspecting the error of a near perfect solution. \todo{Get a better criterion.}
...
\todo{add linear to multiplication}
