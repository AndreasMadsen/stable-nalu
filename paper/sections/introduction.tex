...
We attempt to solve (i-iii) by proposing a much extended arithmetic task, define a bounded evaluation criteria based on convergence and do multiplication of MNIST digits.
...
%Our NMU model is an improvement on the $\text{NAC}_{\bullet}$, where we solve the problems of it not supporting negative and small numbers. 

%NALU uses an input dependent gating mecanism. As shown in our emperical results, section \ref{}, this gating mecanism reduces the success-rate of convergence from $24\%$ to $< 4\%$, for multiplication. This is because the $\text{NAC}_{+}$ is much easier to optimize than $\text{NAC}_{\bullet}$, thus causing $\text{NAC}_{+}$ to be the best estimator early on, even if multiplication is the desired operation, therefore the gating mechanism will switch to just addition making it impossible to learn multiplication by backpropagation. However, for a lucky seed, multiplication can still be learned by pure chance, because the weight matrix is shared between $\text{NAC}_{+}$ and $\text{NAC}_{\bullet}$. So if the weight matrix, though optimization of $\text{NAC}_{+}$ converges, by pure change, to something that makes $\text{NAC}_{\bullet}$ the better estimator, then the gating mechanism will switch to multiplication. This shared weight matrix, is a clear hack, as there is no reason to believe that a desired addition operation should use the same inputs as a desired multiplication operation. This shared gating mechanism, is also what makes NALU much harder to optimize, when the desired operation is addition.
