...
plot.parameter = function(name.parameter, name.label, name.file, name.output, discrete.x.axis=FALSE) {
  ...
  dat.last = dat %>%
    ...
    summarise(
      ...
      parameter.name = last(parameter.name),
      model = last(model),
  ...
  dat.last.rate = dat.last %>%
    group_by(model, operation, parameter, parameter.name) %>%
    summarise(
      size=n(),
  ...
  dat.gather.mean = dat.last.rate %>%
    ...
    select(model, operation, parameter, parameter.name, success.rate, converged.at, sparse.error) %>%
...
input.size = plot.parameter('input.size', 'Input size', '../results/function_task_static_mul_input_size.csv', '../paper/results/simple_function_static_input_size.pdf')
subset.ratio = plot.parameter('subset.ratio', 'Subset ratio', '../results/function_task_static_mul_subset.csv', '../paper/results/simple_function_static_subset.pdf')
overlap.ratio = plot.parameter('overlap.ratio', 'Overlap ratio', '../results/function_task_static_mul_overlap.csv', '../paper/results/simple_function_static_overlap.pdf')
hidden.size = plot.parameter('hidden.size', 'Hidden size', '../results/function_task_static_mul_hidden_size.csv', '../paper/results/simple_function_static_hidden_size.pdf')
interpolation.range = plot.parameter('interpolation.range', 'Interpolation range', '../results/function_task_static_mul_range.csv',  '../paper/results/simple_function_static_range.pdf', discrete.x.axis=F)

p = ggarrange(
  input.size, subset.ratio, overlap.ratio, hidden.size, interpolation.range,
  ncol = 1, nrow = 5,
  common.legend = TRUE, legend = "bottom"
)
print(p)
ggexport(p, filename='../paper/results/simple_function_static_parameters_combined.pdf')
