...
m_{h_\ell} &= \exp\left(\sum_{h_{\ell-1}=1}^{H_{\ell-1}} W_{h_{\ell}, h_{\ell-1}} \log(|x_{h_{\ell-1}}| + \epsilon) \right) \\
...
$$
\begin{aligned}
\frac{\partial a_{h_\ell}}{\partial W_{h_\ell, h_{\ell-1}}} &= x_{h_{\ell-1}} \\
\frac{\partial m_{h_\ell}}{\partial W_{h_\ell, h_{\ell-1}}} &= \exp\left(\sum_{h'_{\ell-1}=1}^{H_{\ell-1}} W_{h_{\ell}, h'_{\ell-1}} \log(|x_{h'_{\ell-1}}| + \epsilon) \right) \log(|x_{h_{\ell-1}}| + \epsilon)
\end{aligned}
$$
...
$$
\frac{\partial b_{h_\ell}}{\partial W_{h_\ell, h_{\ell-1}}} \propto \log(|x_{h_{\ell-1}}| + \epsilon)
$$
...
This expression, I find very odd. As $\sigma(\hat{g}_{h_\ell}) (1 - \sigma(\hat{g}_{h_\ell}))$ is always positive, $\frac{\partial \mathcal{L}}{\partial z_{h_\ell}} (a_{h_\ell} - b_{h_\ell})$ is the only part that controls the sign. The rest just becomes part of the learning rate. Thus we have:
...
In this cause, one could improve exploration by using:
