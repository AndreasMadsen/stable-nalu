...
dataset_train = dataset.fork(seq_length=10).dataloader(batch_size=128)
...
def test_model(dataloader):
    test_loss = 0
    for x_valid, t_valid in dataloader:
        # forward
        y_train = model(x_valid)
        test_loss += criterion(y_train, t_valid).item() * len(t_valid)
