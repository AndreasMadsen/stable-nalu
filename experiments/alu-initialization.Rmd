...
!! E[tanh(\hat{w})] = 0, thus we have a very small gradient for \hat{m}.

Thus, $\frac{\partial \mathcal{L}}{\partial \hat{m}_{h_{\ell-1},h_\ell}}$ is what needs to be expressed, similar to a typical MLP network.

$$
\frac{\partial \mathcal{L}}{\partial \hat{m}_{h_{\ell-1},h_\ell}} = \frac{\partial \mathcal{L}}{\partial z_{h_\ell}} \frac{\partial z_{h_\ell}}{\partial \hat{m}_{h_{\ell-1},h_\ell}} = \delta_{h_\ell} z_{h_{\ell-1}}
$$
...
$$
\mathrm{Var}[W] = \mathrm{E}[\tanh(\hat{W})^2] \mathrm{E}[\tanh(\hat{M})^2] - \mathrm{E}[\tanh(\hat{W})]^2 \mathrm{E}[\tanh(\hat{M})]^2 = \mathrm{E}[\tanh(\hat{W})^2] \mathrm{E}[\tanh(\hat{M})^2]
$$
...
$$
\begin{aligned}
\mathrm{E}[\tanh(\hat{W})^2] &= \int_{-\infty}^{\infty} \sigma(x)^2 f_{U[-r, r]}(x)\ \mathrm{d}x \\
